...
def cargar_config(ruta_json: str = "configuracion.json") -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"ERROR: No se encontró {ruta_json}. Usando configuración simulada.")
        config, _ = generar_datos_de_simulacion(simular_excel=False)
    except json.JSONDecodeError as error_json:
        print(f"ERROR: JSON inválido en {ruta_json}: {error_json}. Usando simulación.")
        config, _ = generar_datos_de_simulacion(simular_excel=False)
    except Exception as error_general:
        print(f"ERROR al cargar configuración desde {ruta_json}: {error_general}. Usando simulación.")
        config, _ = generar_datos_de_simulacion(simular_excel=False)
...
def cargar_salidas_desde_excel(ruta_excel: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for campo in requeridos:
            if campo not in indices:
                print(f"Error: Falta columna {campo}")
                return []
    except Exception as error_headers:
        print(f"Error leyendo encabezados: {error_headers}")
        return []
    ...
    for fila_idx, fila in enumerate(hoja.iter_rows(min_row=2, values_only=True), start=2):
        ...
        try:
            ...
            if fin < inicio:
                print(
                    f"Advertencia (Fila {fila_idx}): Hora Fin ({formatear_hora(fin)}) < Hora Inicio ({formatear_hora(inicio)}). "
                    "Corrigiendo a +30 min."
                )
                fin = inicio + 30
            try:
            ...
            id_excel = f"excel_{fila_idx}"
            id_interno = identificador
            if id_excel in ids_excel:
                print(f"Advertencia: ID duplicado detectado en Excel fila {fila_idx}. Generando ID interno único.")
                id_interno = f"auto_{identificador}"
            else:
                ids_excel.add(id_excel)
            ...
            print(f"Error procesando fila {fila_idx}: {error_fila}")
