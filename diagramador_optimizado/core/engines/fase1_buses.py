...
def resolver_diagramacion_buses(
    ...
    for grupo_nombre, viajes_grupo in items_grupos:
        ...
        # Filtrar viajes ya asignados: usar id o _tmp_id para deduplicación.
        viajes_grupo_filtrados = [
            v for v in viajes_grupo
            if (v.get("id") or v.get("_tmp_id")) not in viajes_asignados
        ]
                ...
                if conexion_valida:
                    ...
                    if mejor_idx is None:
                        ...
                        if desviacion_optimo < mejor_desviacion:
                            ...
                            # Desempate V2.0: maximizar reutilización prefiriendo el bus con hora_disponible
                            # MÁS RECIENTE (más cercana al inicio del viaje actual).
                            # Estrategia "Latest Fit": usar el bus que quedó libre más tarde → deja
                            # los buses más "viejos" disponibles para viajes con restricciones estrictas,
                            # minimizando la necesidad de crear nuevos buses.
                            ...
                            # Preferir el bus con hora_disponible más reciente (mayor valor)
            ...
            for i in range(len(buses_grupo)):
                ...
                for j in range(len(buses_grupo)):
                    ...
                    if primer_j["inicio"] <= ultimo_i["fin"]:
                        continue
                    es_conexion, _detalle = gestor.evaluar_conexion_bus(
                        ultimo_i,
                        primer_j,
                        devolver_detalle=True,
                    )
                    if es_conexion:
                        # ANTITELETRANSPORTACIÓN: destino último ev i debe == origen primer ev j
                        viajes_j = bus_j.get("viajes", [])
                        viajes_i = bus_i.get("viajes", [])
                        ultimo_ev_i = viajes_i[-1] if viajes_i else None
                        primer_ev_j = viajes_j[0] if viajes_j else None
                        dest_i = ultimo_ev_i.get("destino") if (ultimo_ev_i and isinstance(ultimo_ev_i, dict)) else None
                        orig_j = primer_ev_j.get("origen") if (primer_ev_j and isinstance(primer_ev_j, dict)) else None
                        if not _mismo_lugar_nodo(dest_i or "", orig_j or "", nombres_depositos_upper, deposito_a_terminal):
                            continue  # Teletransportación: no fusionar
                        # Validar orden temporal: primer evento de bus_j debe empezar después del último de bus_i
                        fin_i = ultimo_ev_i.get("fin") if ultimo_ev_i else None
                        inicio_j = primer_ev_j.get("inicio") if primer_ev_j else None
    ...
    for bloque in todos_los_bloques:
        for item in bloque:
            vid = item.get("id") or item.get("_tmp_id")
            if vid is not None:
                ids_en_bloques.add(vid)
                ids_en_bloques.add(str(vid))
    faltantes_bloques = [
        v for v in viajes_comerciales
        if (v.get("id") or v.get("_tmp_id")) not in ids_en_bloques
        and str(v.get("id") or v.get("_tmp_id")) not in ids_en_bloques
    ]
    if faltantes_bloques:
        ...
        for viaje in faltantes_bloques:
            ...
            viajes_asignados.add(viaje.get("id") or viaje.get("_tmp_id"))
            if verbose:
    ...
    if max_buses_check and len(todos_los_bloques) > max_buses_check:
        print(
            f"[ADVERTENCIA] Se requieren {len(todos_los_bloques)} buses y el límite configurado es {max_buses_check}. "
            "Revisa vacíos/paradas o ajusta el parámetro si corresponde."
        )
...
def _resolver_diagramacion_buses_cp_sat(
    ...
    if num_buses > max_buses and status_str == "OPTIMAL":
        ...
        print(
            f"ADVERTENCIA: Se generaron {num_buses} buses pero el máximo configurado es {max_buses}. "
            "Revisa reglas de paradas/vacíos o ajusta 'max_buses'."
        )
